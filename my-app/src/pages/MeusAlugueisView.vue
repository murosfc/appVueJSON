        ...
        <v-row justify="center" data-app>
            <v-dialog v-model="modalShow" max-width="700px" max-length="500px">
                ...
                <v-container>
                    ...
                    <v-row>
                        <v-data-table dense :headers="headerJogos" :items="jogoInThisAluguel" item-key="id" class="elevation-1"></v-data-table>
                    </v-row>                            
...
export default ({
    ...
    methods: {
        showAluguel(item){                     
            ...
            .then((response) => {                
                jogoInThisAluguelAux = response.data;
                console.log(jogoInThisAluguelAux);
            })                       
            .catch((error)=> console.log(error)); 

            for (var i=0; i<this.jogoInThisAluguelAux.length; i++){                
                var game = this.games.find(g => g.id == this.jogoInAluguel[i].id);
                console.log(game);
        ...
        inicializa() {
            ...
            if (this.$parent.session.cliente){
                console.log( console.log(this.$parent.dadosLogin.id));
                axios.get('http://localhost:3000/alugueis', {
                params: {                   
